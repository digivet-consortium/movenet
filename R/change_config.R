...
change_config <- function(...){
  ...
  if(length(opts)>0){
    ...
    if(("movedata_fileopts.date_format" %in% optnames) & (!grepl("%(Y|y|AD|D|F|x|s)|^$",opts["movedata_fileopts.date_format"]))){
    ...
    if(("holdingdata_fileopts.date_format" %in% optnames) & (!grepl("%(Y|y|AD|D|F|x|s)|^$",opts["holdingdata_fileopts.date_format"]))){
