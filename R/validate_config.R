...
validate_config_datatype <- function(yamlfile){
  ...
  if(length(fileopts) > 0){
    ...
    if (any(sapply(opts_dateformat,function(x){all(x %in% names(fileopts) && !grepl("%(Y|y|AD|D|F|x|s)|^$",fileopts[x]))}))){
      wrongdateformat_names <- opts_dateformat[which(opts_dateformat %in% names(fileopts) & !grepl("%(Y|y|AD|D|F|x|s)|^$",fileopts[opts_dateformat]))]
