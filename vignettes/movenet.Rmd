...
data, they may also follow different formatting customs for, for
...
-   extracting columns with minimally required data, corresponding to "from" (originating holding identifier), "to" (destination holding identifier), "date", and "weight" (movement quantity). The resulting data tibble contains these columns in this
    specific order.
...
coordinates, "EPSG code" and "country code" are required file options, and "coord_x" 
...
-   If the loaded configurations include values for columns with geographical 
    coordinates ("coord_x" and "coord_y"), these columns are checked to contain 
    numeric data, and are then reformatted to a single simple feature (sf) geometry 
    list-column.
    
-   If the loaded configurations include a value for a column "herd size", this 
